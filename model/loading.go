...
func AboutLoading() {
	...
	bytes, err := ioutil.ReadFile(filepath.Join(ROOT, "page/about.md"))
	if err == nil {
...
func LinkLoading() {
	...
	bytes, err := ioutil.ReadFile(filepath.Join(ROOT, "page/links.yaml"))
	if err == nil {
		if err = yaml.Unmarshal(bytes, &Link_.links); err != nil {
			panic(err)
		}
	}
	bytes, err = ioutil.ReadFile(filepath.Join(ROOT, "page/links.md"))
	if err == nil {
